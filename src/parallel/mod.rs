...
pub fn compute_asap_schedule_parallel(circuit: &mut Circuit) -> Result<()> {
    ...
    // For now, just use the sequential algorithm and simulate parallelism
    // This will give consistent results while we develop the parallel implementation
...
pub fn create_naive_mapping_parallel(circuit: &mut Circuit) -> Result<CrossbarMapping> {
    ...
    for level in 0..=max_level {
        ...
        let gate_info: Vec<_> = gates_at_level.iter().enumerate().map(|(level_idx, &gate_idx)| {
            let gate = &circuit.gates[gate_idx];
            let column = column_indices[level_idx] as usize;
            
            (gate_idx, gate.out, gate.fanin, gate.inputs[0], 
             if gate.fanin > 1 { gate.inputs[1] } else { -1 }, 
             column, gate.asap_level)
        }).collect();
...
pub fn compute_list_schedule_parallel(circuit: &mut Circuit) -> Result<()> {
    ...
    let mut sorted_gates: Vec<(usize, i32)> = circuit.gates.iter().enumerate()
        .map(|(i, g)| (i, g.slack))
        .collect();
    ...
    for (gate_idx, _) in sorted_gates {
        ...
        // Add to resource map
        resource_map.insert(start_time, resource_map.get(&start_time).map(|c| *c + 1).unwrap_or(1));
    }
    
    // Find the maximum list time and resource usage
    let max_time = resource_map.iter().map(|entry| *entry.key()).max().unwrap_or(0);
...
pub fn create_compact_mapping_parallel(circuit: &mut Circuit) -> Result<CrossbarMapping> {
    ...
    for time in 0..=max_list_time {
        ...
        let gate_info: Vec<_> = gates_at_time.iter().enumerate().map(|(idx, &gate_idx)| {
            let gate = &circuit.gates[gate_idx];
